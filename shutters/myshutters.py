...
class LCMYW(ShutterBase):
    def __init__(self,block_size,cfa_size, w_zhi,alpha,test=False, resume=False,init=''):
        ...
        # """
        # 柯达 4x4CFA
        cmyw_mask = torch.zeros(size=(4,cfa_size[0],cfa_size[1]),dtype=torch.float32,device=device)#创建cmyW CFA
        # cmyw_mask[:,...]=0.5
        cmyw_mask[0,::4,2::4]=self.w_zhi
        cmyw_mask[0,1::4,3::4]=self.w_zhi

        cmyw_mask[1,::4,::4]=self.w_zhi
        cmyw_mask[1,1::4,1::4]=self.w_zhi
        cmyw_mask[1,2::4,2::4]=self.w_zhi
        cmyw_mask[1,3::4,3::4]=self.w_zhi

        cmyw_mask[2,2::4,::4]=self.w_zhi
        cmyw_mask[2,3::4,1::4]=self.w_zhi

        cmyw_mask[3,::2,1::2]=self.w_zhi
        cmyw_mask[3,1::2,::2]=self.w_zhi
        # """
        ...
        # 新代码（随机初始化）
        w = torch.rand(size=(self.cfa_size[0] * self.cfa_size[1], 4, 1, 1), device=device) * self.w_zhi
        w = F.softmax(w, dim=1)
    ...
    def forward(self, image, train=False):
        ...
        if train:
            ...
            # """
            # 柯达 4x4CFA
            cmyw_mask = torch.zeros(size=(4, self.cfa_size[0], self.cfa_size[1]), dtype=torch.float32,
                                    device=device)  # 创建cmyW CFA
            # cmyw_mask[:,...]=0.5
            cmyw_mask[0, ::4, 2::4] = self.w_zhi
            cmyw_mask[0, 1::4, 3::4] = self.w_zhi
            # green channels
            ...
            mask = torch.zeros(
                size=(self.cfa_size[0] * self.cfa_size[1], image.shape[1], image.shape[2], image.shape[3]),
                device=device)  #不再是128，是原尺寸
            ...
            for i in range(self.cfa_size[0]):
                for j in range(self.cfa_size[1]):
                    w[group_count] = cmyw_mask[:, i, j].reshape(4, 1, 1)  # 对w进行初始化
                    mask[group_count, :, i::self.cfa_size[0], j::self.cfa_size[1]] = 1  # 对mask进行初始化
                    group_count += 1

            cfa_image,cfa=Proxy_function(cfa_image=cfa_image,cfa=cfa,init=self.init,image=image,w=self.w,
            #                              mask=self.mask,
                                         mask=mask,
                                         alpha=self.alpha)
        else:
            # '''
            mask=torch.zeros(size=(self.cfa_size[0]*self.cfa_size[1],image.shape[1],image.shape[2],image.shape[3]),
                             device=device)
            group_count=0
            for i in range(self.cfa_size[0]):
                for j in range(self.cfa_size[1]):
