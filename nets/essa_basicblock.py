...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        super().__init__()
        self.dim = dim
        self.wi1ndow_size = window_size  # Wh, Ww
        ...
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((head_dim - 1) * (head_dim - 1), num_heads))  # 2*Wh-1 * 2*Ww-1, nH

        # get pair-wise relative position index for each token inside the window
        coords_h = torch.arange(head_dim)
        coords_w = torch.arange(head_dim)
        coords = torch.stack(torch.meshgrid([coords_h, coords_w]))  # 2, Wh, Ww
        coords_flatten = torch.flatten(coords, 1)  # 2, Wh*Ww
        relative_coords = coords_flatten[:, :, None] - coords_flatten[:, None, :]  # 2, Wh*Ww, Wh*Ww
        relative_coords = relative_coords.permute(1, 2, 0).contiguous()  # Wh*Ww, Wh*Ww, 2
        relative_coords[:, :, 0] += head_dim - 1  # shift to start from 0
        relative_coords[:, :, 1] += head_dim - 1
        relative_coords[:, :, 0] *= 2 * head_dim - 1
    ...
    def forward(self, x, mask=None):
        ...
        a = torch.mean(q, dim=3, keepdim=True)
        q = q - a
        a = torch.mean(k, dim=3, keepdim=True)
        k = k - a
        q2 = torch.pow(q, 2)
        q2s = torch.sum(q2, dim=3, keepdim=True)
        k2 = torch.pow(k, 2)
        k2s = torch.sum(k2, dim=3, keepdim=True)
        t1 = v
        k2 = torch.nn.functional.normalize((k2 / (k2s + 1e-7)), dim=-2)
        q2 = torch.nn.functional.normalize((q2 / (q2s + 1e-7)), dim=-1)
        t2 = (k2.transpose(-2, -1) @ v)
        ...
        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(
            C // self.num_heads, C // self.num_heads, -1)  # Wh*Ww,Wh*Ww,nH
        relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous()  # nH, Wh*Ww, Wh*Ww
        t2 = t2 + relative_position_bias.unsqueeze(0)

        if mask is not None:
            nW = mask.shape[0]
            t2 = t2.view(B_ // nW, nW, self.num_heads, N, N) / mask.unsqueeze(1).unsqueeze(0)
            t2 = t2.view(-1, self.num_heads, N, N)
...
class SwinTransformerBlock(nn.Module):
    ...
    def calculate_mask(self, x_size):
        ...
        attn_mask = attn_mask.masked_fill(attn_mask != 0, float(100000.0)).masked_fill(attn_mask == 0, float(1.0))
...
class RSTCABlock(nn.Module):
    def __init__(self, patches_resolution, stage_id,
                 depths, num_heads, nc=12, window_size=8,
        ...
        self.st_blocks = BasicLayer(dim=self.dim,
                                    ...
                                    drop_path=dpr[sum(depths[:stage_id]):sum(depths[:stage_id + 1])],
                                    norm_layer=norm_layer,
                                    downsample=None,
...
class RSTCAGroup(nn.Module):
    def __init__(self, patches_resolution, stage_id, depths, num_heads, nc=12, window_size=8, **kwargs):
        super(RSTCAGroup, self).__init__()
        
        self.dim = nc
        
        self.rstcab = RSTCABlock(patches_resolution, stage_id, depths, num_heads, nc, window_size)
        
        self.conv = conv(self.dim, self.dim, bias=True, mode='C')
        
    def forward(self, input, x_size):

        x = self.rstcab(input, x_size)
        H, W = x_size
        B, L, C = input.shape
        assert L == H * W, "input feature has wrong size"
...
class Body(nn.Module):
    def __init__(self, patches_resolution, depths, num_heads, nc=12, window_size=8, **kwargs):
        super(Body, self).__init__()
        
        self.rg = nn.ModuleList([
            RSTCAGroup(patches_resolution, stage_id, depths, num_heads, nc, window_size) 
