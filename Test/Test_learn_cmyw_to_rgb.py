...
import os
import lpips
from piq import ssim, psnr   # psnr 也可以直接用，但您已有 summary_utils.get_psnr，可自行选择
from place2.SIED_CMYW_GT_PatchDataset import CMYW_RGB_PatchDataset
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_epoch',type=str,default='1_349')
    ...
    #SIED  数据集测试
    # ---------- 测试集（目录模式） ----------
    test_cmyw_dir = r'..\data\Sony_test_data\0.0001noisy'
    test_gt_dir = r'..\data\Sony_test_data\gt'
    dataset_test = CMYW_RGB_PatchDataset(
        cmyw_path=test_cmyw_dir,
        gt_path=test_gt_dir,
        transform=None,  # 验证时不增强
        crop_size=None  #
    )
    ...
    model.to(device)
    model.eval()
    lpips_model = lpips.LPIPS(net='alex').to(device)  # 也可用 'vgg'
    lpips_model.eval()
    # 在测试前获取 GT 文件名列表

    gt_filenames = sorted([f for f in os.listdir(test_gt_dir)
                           if f.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tif'))])
    print(f"GT 文件数量: {len(gt_filenames)}")

    # 确保顺序一致（一般数据集内部也是 sorted(os.listdir(...))）
    assert len(gt_filenames) == len(dataset_test), "文件名数量与数据集样本数不匹配！"
    with torch.no_grad():
        psnr_list = []
        ssim_list = []
        lpips_list = []
        for step, (model_input, gt) in enumerate(test_dataloader, 1):
            ...
            deblur = torch.clamp(deblur, 0, 1)  # 确保 [0,1]
            gt_rgb = gt[:, :3, :, :]
            ...
            # 使用对应的 GT 文件名
            gt_filename = gt_filenames[step - 1]
